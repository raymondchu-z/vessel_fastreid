...
class Visualizer:
    ...
    def get_matched_result(self, q_index):
        ...
        cmc = self.matches[q_index][keep].astype(np.int8)
        sort_idx = order[keep]
        return cmc, sort_idx
    ...
    def vis_roc_curve(self, output):
        ...
        for i, q in enumerate(self.q_pids):
            ...
            ind_pos = np.where(cmc == 1)[0].astype(np.int32)
            q_dist = self.dist[i]
            pos.extend(q_dist[sort_idx[ind_pos]])

            ind_neg = np.where(cmc == 0)[0].astype(np.int32)
            neg.extend(q_dist[sort_idx[ind_neg]])
        
        scores = np.hstack((pos, neg)) #175G
        labels = np.hstack((np.zeros(len(pos)), np.ones(len(neg)))).astype(np.int8)
        

        fpr, tpr, thresholds = metrics.roc_curve(labels, scores)#这里爆内存
    ...
    def save_roc_curve(self, output):
        PathManager.mkdirs(output)
        pos, neg = [], []
        for i, q in enumerate(self.q_pids):
            cmc, sort_idx = self.get_matched_result(i)  # remove same id in same camera
            ...
            ind_neg = np.where(cmc == 0)[0].astype(np.int32)
            neg.extend(q_dist[sort_idx[ind_neg]])
        print("calculating scores and labels")
        scores = np.hstack((pos, neg)) #175G
        labels = np.hstack((np.zeros(len(pos)), np.ones(len(neg)))).astype(np.int8)
        print("save pos and neg")
        self.save_pos_neg(output, pos, neg)
        del pos, neg
        gc.collect()
        print("gc.collect()")
        fpr, tpr, thresholds = metrics.roc_curve(labels, scores)#这里爆内存
        self.save_fpr_tpr(output, fpr, tpr)
        # self.plot_roc_curve(fpr, tpr)
        # filepath = os.path.join(output, "roc.jpg")
        # plt.savefig(filepath)
        # self.plot_distribution(pos, neg)
    ...
    @staticmethod
    def save_pos_neg(output, pos, neg):
        results = {
            "pos": np.asarray(pos),
            "neg": np.asarray(neg),
        }
        with open(os.path.join(output, "pos_neg.pickle"), "wb") as handle:
            pickle.dump(results, handle, protocol=pickle.HIGHEST_PROTOCOL)

    @staticmethod
    def save_fpr_tpr(output, fpr, tpr):
        results = {
            "fpr": np.asarray(fpr),
            "tpr": np.asarray(tpr),
        }
        with open(os.path.join(output, "fpr_tpr.pickle"), "wb") as handle:
            pickle.dump(results, handle, protocol=pickle.HIGHEST_PROTOCOL)
    @staticmethod
